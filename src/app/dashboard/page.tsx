  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
        <span style={{ fontSize: '12px', fontWeight: 800, color: 'var(--muted-bright)' }}>
          ⚡ XP
        </span>
        <span style={{ fontSize: '12px', fontWeight: 800, color: 'var(--fg-secondary)' }}>
          {xp % max} / {max}
        </span>
      </div>
...
function Dashboard() {
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'var(--bg)', fontFamily: 'var(--font-ui)', position: 'relative', overflow: 'hidden' }}>
      {/* Background blobs */}
      <div style={{ position: 'fixed', top: '-120px', right: '-120px', width: '420px', height: '420px', borderRadius: '50%', background: 'rgba(88,204,2,0.08)', filter: 'blur(60px)', pointerEvents: 'none' }} />
      <div style={{ position: 'fixed', bottom: '-100px', left: '-100px', width: '340px', height: '340px', borderRadius: '50%', background: 'rgba(28,176,246,0.07)', filter: 'blur(50px)', pointerEvents: 'none' }} />
      ...
      <div style={{ maxWidth: '600px', margin: '0 auto', padding: '0 1rem 6rem' }}>
        ...
        <div style={{
          ...
          <div>
            <p style={{ fontSize: '12px', fontWeight: 800, letterSpacing: '0.12em', color: 'var(--muted)', textTransform: 'uppercase' }}>
              言語道場
            </p>
            <h1 style={{ fontSize: '26px', fontWeight: 900, color: 'var(--fg)', fontFamily: 'var(--font-display)', lineHeight: 1.1 }}>
              Gengo Dojo 🥋
            </h1>
          </div>
          {agent && (
            <div style={{
              ...
              <div style={{
                ...
                background: '#fff3d0', border: '2.5px solid #ffd966',
                borderRadius: '99px', padding: '6px 12px',
                boxShadow: '0 4px 0 #ffd966',
                animation: 'glow-pulse 2.5s ease-in-out infinite',
              }}>
                <span style={{ fontSize: '16px', animation: 'flicker 1.5s ease-in-out infinite' }}>🔥</span>
                <span style={{ fontFamily: 'var(--font-display)', fontSize: '15px', fontWeight: 700, color: '#a05600' }}>
                  {agent.streakDays}
                </span>
              </div>
              <div style={{
                background: '#e8f8ff', border: '2.5px solid #74d4ff',
                borderRadius: '99px', padding: '6px 12px',
                boxShadow: '0 4px 0 #74d4ff',
              }}>
                <span style={{ fontSize: '14px', fontWeight: 800, color: '#0068a0' }}>
                  Lv.{Math.floor(xp / 500) + 1}
                </span>
              </div>
            </div>
          )}
        ...
        {agent && ready && (
          <div style={{
            background: '#fff', borderRadius: '16px', border: '2.5px solid var(--border-dark)',
        ...
        <div style={{
          marginBottom: '1.5rem',
          animation: ready ? 'fadeUp 0.5s ease 0.18s both' : 'none',
        }}>
          <p style={{ fontSize: '12px', fontWeight: 800, letterSpacing: '0.1em', color: 'var(--muted)', textTransform: 'uppercase', marginBottom: '12px' }}>
            🗺️ Mission Path
          </p>
          <div style={{ position: 'relative', padding: '0 0 0 60px' }}>
            <div style={{
              position: 'absolute', left: '24px', top: '12px', bottom: '12px', width: '3px',
              background: 'linear-gradient(to bottom, var(--green), var(--blue-light))',
              borderRadius: '2px', transformOrigin: 'top',
              animation: ready ? 'lineGrow 0.6s cubic-bezier(0.16,1,0.3,1) 0.28s both' : 'none',
            }} />
              ...
              <div key={m.label} style={{
                ...
                <button
                  ...
                  style={{
                    ...
                    border: `3px solid ${m.active ? m.color : 'var(--border-dark)'}`,
                    background: m.active ? m.bg : '#f0f0f0',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontSize: '22px', cursor: m.active ? 'pointer' : 'default',
                ...
                <button
                  onClick={() => m.active && router.push(m.route)}
                  className={m.active ? 'mission-row-btn' : ''}
                  style={{
                    flex: 1,
                    display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                    padding: '13px 16px',
                    borderRadius: '16px',
                    background: m.active ? '#fff' : '#f7f7f7',
                    border: `2.5px solid ${m.active ? m.color + '55' : 'var(--border-dark)'}`,
        ...
        {/* ── Study tools ── */}
        <p style={{
          fontSize: '12px', fontWeight: 800, letterSpacing: '0.1em', color: 'var(--muted)',
          textTransform: 'uppercase', marginBottom: '12px',
          animation: ready ? 'fadeUp 0.4s ease 0.33s both' : 'none',
        }}>
          🎮 Study Tools
        </p>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px', marginBottom: '1.5rem' }}>
          {[
            { emoji: '📖', label: 'Word List',  sub: `${words.length} words · ${dueCount} due`, route: '/words',      color: 'var(--blue)',   bg: 'var(--blue-light)' },
            { emoji: '🃏', label: 'Flashcards', sub: 'SRS review',                               route: '/flashcards', color: 'var(--green)',  bg: 'var(--green-light)' },
            { emoji: '🧩', label: 'Quiz',       sub: 'Multiple choice',                          route: '/quiz',       color: 'var(--blue)',   bg: 'var(--blue-light)' },
            { emoji: '💀', label: 'Survival',   sub: '3 lives · timed',                          route: '/survival',   color: 'var(--red)',    bg: 'var(--red-light)' },
            { emoji: '✍️', label: 'Write',      sub: 'Canvas practice',                          route: '/write',      color: 'var(--pink)',   bg: 'var(--purple-light)' },
            { emoji: '🎤', label: 'Shadow',     sub: 'Pronunciation',                            route: '/shadow',     color: 'var(--orange)', bg: 'var(--orange-light)' },
            ...(profile?.targetLang === 'ja' || profile?.targetLang === 'ko'
              ? [{ emoji: 'あ', label: profile.targetLang === 'ja' ? 'Kana' : 'Hangul', sub: 'Characters', route: '/kana', color: 'var(--green)', bg: 'var(--green-light)' }]
              : []),
          ].map((tool, i) => (
            <button
              key={tool.label}
              ...
              style={{
                ...
                borderRadius: '16px',
                background: '#fff',
                border: `2.5px solid ${tool.color}55`,
                boxShadow: `0 5px 0 ${tool.color}55`,
                cursor: 'pointer', fontFamily: 'var(--font-ui)',
                transition: 'transform 0.1s ease, box-shadow 0.1s ease, top 0.1s ease',
                textAlign: 'left',
                position: 'relative', top: 0,
                animation: ready ? `bounceIn 0.5s cubic-bezier(0.34,1.56,0.64,1) ${0.36 + i * 0.05}s both` : 'none',
              }}
            >
              <span style={{
                fontSize: '26px', width: '44px', height: '44px',
                borderRadius: '12px', background: tool.bg,
                border: `2px solid ${tool.color}55`,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                flexShrink: 0,
                fontFamily: tool.emoji === 'あ' ? '"Noto Sans JP",serif' : 'inherit',
              ...
              <div>
                ...
                <p style={{ fontSize: '12px', color: 'var(--muted)', fontWeight: 600 }}>{tool.sub}</p>
              </div>
            </button>
          ))}
        </div>
        ...
        {dueCount > 0 && (
          <div style={{
            marginBottom: '1.5rem',
            animation: ready ? 'fadeUp 0.4s ease 0.48s both' : 'none',
          }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
              <p style={{ fontSize: '12px', fontWeight: 800, letterSpacing: '0.1em', color: 'var(--muted)', textTransform: 'uppercase' }}>
                ⏰ Due for Review
              </p>
              <span className="pill pill-orange" style={{
                background: 'var(--orange-light)', color: '#a05600',
            ...
            <div style={{ background: '#fff', borderRadius: '16px', border: '2.5px solid var(--border-dark)', boxShadow: '0 5px 0 var(--border-dark)', overflow: 'hidden' }}>
              {dueWords.slice(0, 5).map((w, i) => (
                <div key={w.id} style={{
                  display: 'flex', alignItems: 'center', gap: '12px',
                  padding: '11px 16px',
                  borderBottom: i < Math.min(dueWords.length, 5) - 1 ? '2px solid var(--border)' : 'none',
                  borderLeft: '4px solid var(--orange)',
                  transition: 'background 0.15s ease',
                  animation: ready ? `fadeRight 0.35s ease ${0.52 + i * 0.06}s both` : 'none',
                }} className="due-row">
                  <span style={{ fontSize: '22px', minWidth: '30px', fontFamily: '"Noto Sans JP","Noto Sans SC",serif', color: 'var(--fg)' }}>
                    {w.kanji}
                  </span>
        ...
        <div style={{
          marginBottom: '1.5rem',
          animation: ready ? 'fadeUp 0.4s ease 0.52s both' : 'none',
        }}>
          <p style={{ fontSize: '12px', fontWeight: 800, letterSpacing: '0.1em', color: 'var(--muted)', textTransform: 'uppercase', marginBottom: '10px' }}>
            ✨ Generate Vocabulary
          </p>
          {topics.length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '8px' }}>
              {topics.map(t => (
                <button key={t} className="btn btn-blue"
                  style={{ fontSize: '13px', padding: '8px 16px' }}
                  disabled={!!generating} onClick={() => generateWords(t)}>
                  {generating === t
                    ? <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><Spinner size={14} color="#fff" /> Generating…</span>
                    : `+ More ${t}`}
                </button>
              ))}
            </div>
        ...
        {!editingProfile ? (
          <div style={{ animation: ready ? 'fadeUp 0.4s ease 0.56s both' : 'none' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                ...
                <button className="btn btn-red" style={{ fontSize: '13px', padding: '7px 14px' }}
          ...
          <div style={{ animation: 'fadeUp 0.25s ease both' }}>
            <p style={{ fontSize: '12px', fontWeight: 800, letterSpacing: '0.1em', color: 'var(--muted)', textTransform: 'uppercase', marginBottom: '1.25rem' }}>
              ✏️ Edit Profile
            </p>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  {NATIVE_LANGUAGES.map(l => (
                    <button key={l.code} onClick={() => setNewNativeLang(l.code)} style={{
                      ...
                      borderColor: newNativeLang === l.code ? 'var(--green)' : 'var(--border-dark)',
                      background: newNativeLang === l.code ? 'var(--green-light)' : '#fff',
                      color: newNativeLang === l.code ? 'var(--green-dark)' : 'var(--fg)',
                      cursor: 'pointer', fontFamily: 'var(--font-ui)', fontSize: '13px', textAlign: 'left', width: '100%',
                      fontWeight: 700,
                      boxShadow: newNativeLang === l.code ? '0 4px 0 var(--green-dark)' : '0 4px 0 var(--border-dark)',
                  ...
                  {TARGET_LANGUAGES.map(l => (
                    <button key={l.code} onClick={() => setNewTargetLang(l.code)}
                      ...
                      style={{
                        display: 'flex', alignItems: 'center', gap: '8px', padding: '9px 12px',
                        borderRadius: '12px', border: '2.5px solid',
                        borderColor: newTargetLang === l.code ? 'var(--blue)' : 'var(--border-dark)',
                        background: newTargetLang === l.code ? 'var(--blue-light)' : '#fff',
                        color: newTargetLang === l.code ? 'var(--blue-dark)' : 'var(--fg)',
                        ...
                        fontFamily: 'var(--font-ui)', fontSize: '13px', textAlign: 'left', width: '100%',
                        fontWeight: 700,
                        boxShadow: newTargetLang === l.code ? '0 4px 0 var(--blue-dark)' : '0 4px 0 var(--border-dark)',
                        transition: 'all 0.12s ease',
                      }}>
                      <span>{l.flag}</span><span style={{ flex: 1 }}>{l.label}</span>
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '1.25rem' }}>
              {(['beginner','intermediate','advanced'] as const).map(l => (
                <button key={l} onClick={() => setNewLevel(l)} style={{
                  flex: 1, padding: '9px 8px', borderRadius: '12px', border: '2.5px solid',
                  borderColor: newLevel === l ? 'var(--purple)' : 'var(--border-dark)',
                  background: newLevel === l ? 'var(--purple-light)' : '#fff',
                  color: newLevel === l ? 'var(--purple-dark)' : 'var(--fg-secondary)',
                  cursor: 'pointer', fontFamily: 'var(--font-ui)', fontSize: '13px', fontWeight: 700,
                  boxShadow: newLevel === l ? '0 4px 0 var(--purple-dark)' : '0 4px 0 var(--border-dark)',
                  transition: 'all 0.12s ease',
                }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '7px', marginBottom: '1.5rem' }}>
              {INTERESTS.map(i => (
                <button key={i} onClick={() => setNewInterests(prev => prev.includes(i) ? prev.filter(x => x !== i) : [...prev, i])}
                  style={{
                    padding: '6px 14px', borderRadius: '99px', border: '2.5px solid', fontSize: '13px',
                    borderColor: newInterests.includes(i) ? 'var(--green)' : 'var(--border-dark)',
                    background: newInterests.includes(i) ? 'var(--green-light)' : '#fff',
                    color: newInterests.includes(i) ? '#2a7a00' : 'var(--fg-secondary)',
                    cursor: 'pointer', fontFamily: 'var(--font-ui)', fontWeight: 700,
                    boxShadow: newInterests.includes(i) ? '0 3px 0 var(--green-dark)' : '0 3px 0 var(--border-dark)',
                    transition: 'all 0.12s ease',
                  }}>
                  {i}
                </button>
            ...
            {(newTargetLang !== profile?.targetLang || newNativeLang !== profile?.nativeLang) && (
              <div style={{ fontSize: '13px', fontWeight: 700, color: '#a05600', background: 'var(--orange-light)', padding: '10px 14px', borderRadius: '12px', marginBottom: '1rem', border: '2px solid var(--orange)' }}>
                ⚡ Switching language will change your word list. Previous words are saved.
              </div>
            )}
      ...
      <style>{`
        @keyframes glow-pulse {
          0%,100% { box-shadow: 0 4px 0 #ffd966; }
          50%     { box-shadow: 0 4px 16px rgba(255,200,0,0.5), 0 4px 0 #ffd966; }
        }
        @keyframes flicker {
        ...
        .mission-node:hover  { transform: scale(1.12); }
        .mission-node:active { transform: scale(0.95); }
        .mission-row-btn:hover  { background: #fafffe !important; transform: translateY(-2px); box-shadow-offset-y: 7px !important; }
