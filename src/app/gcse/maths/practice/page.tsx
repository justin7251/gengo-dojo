...
function MathsPractice() {
  ...
  if (phase === 'setup') return (
    <Screen>
      ...
      {/* Tier */}
      <p style={{ fontSize: '11px', letterSpacing: '0.1em', color: 'rgba(255,255,255,0.3)', marginBottom: '8px' }}>TIER</p>
      <div style={{ display: 'flex', gap: '8px', marginBottom: '1.5rem' }}>
        {(['Foundation', 'Higher'] as Tier[]).map(t => (
          <button key={t} onClick={() => setTier(t)} style={{
            ...
            background: tier === t ? 'rgba(55,138,221,0.12)' : 'rgba(255,255,255,0.04)',
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '2rem' }}>
        {(['P1', 'P2', 'P3'] as Paper[]).map(p => (
          <button key={p} onClick={() => setPaper(p)} style={{
            ...
            background: paper === p ? 'rgba(55,138,221,0.12)' : 'rgba(255,255,255,0.04)',
          }}>
            <p style={{ fontSize: '13px', fontWeight: 600, color: '#fff', marginBottom: '2px' }}>
              {PAPER_META[p].label}
            </p>
            <p style={{ fontSize: '10px', color: 'rgba(255,255,255,0.4)', lineHeight: 1.3 }}>
              {PAPER_META[p].desc}
            </p>
          </button>
        ))}
      ...
      <div style={{ padding: '12px 14px', background: 'rgba(55,138,221,0.07)', borderRadius: '10px', border: '1px solid rgba(55,138,221,0.18)', marginBottom: '2rem', fontSize: '12px', color: 'rgba(255,255,255,0.4)' }}>
  ...
  if (phase === 'active') return (
    <Screen>
      {/* Sticky timer */}
      <div style={{ position: 'sticky', top: 0, zIndex: 10, background: '#050a18', paddingBottom: '10px', marginBottom: '1rem' }}>
        ...
        {questions.map(q => (
          <div key={q.number} style={{ background: 'rgba(0,0,0,0.3)', borderRadius: '14px', padding: '16px', border: '1px solid rgba(255,255,255,0.07)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
              ...
              <span style={{ fontSize: '10px', padding: '2px 6px', borderRadius: '99px', background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.35)' }}>{q.subtopic}</span>
            </div>
            <p style={{ fontSize: '15px', color: 'rgba(255,255,255,0.9)', lineHeight: 1.7, marginBottom: '8px', whiteSpace: 'pre-wrap', fontFamily: q.question.match(/[=^²³√]/) ? 'var(--font-mono)' : 'var(--font-ui)' }}>
              {q.question}
            </p>
            <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.25)', fontStyle: 'italic', marginBottom: '10px' }}>💡 {q.hint}</p>
            <textarea
              value={answers[q.number] ?? ''}
              onChange={e => setAnswers(p => ({ ...p, [q.number]: e.target.value }))}
              placeholder={`Show working for Q${q.number}…`}
              rows={4}
              style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '10px', color: '#fff', fontFamily: 'var(--font-mono)', fontSize: '13px', padding: '10px 12px', outline: 'none', resize: 'vertical', lineHeight: 1.6 }}
            />
          </div>
        ))}
      </div>
    ...
    return (
      <Screen>
        ...
        <div style={{
          ...
          <p style={{ fontSize: '11px', letterSpacing: '0.1em', color: 'rgba(255,255,255,0.3)', marginBottom: '6px' }}>AQA GRADE ESTIMATE</p>
          <p style={{ fontSize: '64px', fontWeight: 700, color: '#fff', lineHeight: 1, fontFamily: 'var(--font-mono)' }}>{grade}</p>
          <p style={{ fontSize: '15px', color: 'rgba(255,255,255,0.45)', marginTop: '8px', fontFamily: 'var(--font-mono)' }}>
            ...
            return (
              <div key={i} style={{ background: 'rgba(0,0,0,0.3)', borderRadius: '14px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.07)' }}>
                ...
                <div style={{ padding: '12px 14px' }}>
                  <p style={{ fontSize: '10px', letterSpacing: '0.08em', color: 'rgba(255,255,255,0.25)', marginBottom: '6px' }}>MODEL SOLUTION</p>
                  <pre style={{ fontSize: '13px', color: 'rgba(255,255,255,0.75)', lineHeight: 1.7, whiteSpace: 'pre-wrap', fontFamily: 'var(--font-mono)', background: 'rgba(255,255,255,0.04)', padding: '10px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.07)', marginBottom: '8px', margin: '0 0 8px 0' }}>
                    {r.working}
                  </pre>
