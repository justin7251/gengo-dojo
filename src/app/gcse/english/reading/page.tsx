...
function GCSEReading() {
  ...
  if (phase === 'setup') return (
    <Screen>
      ...
      <div style={{ flex: 1 }}>
        <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.5)', marginBottom: '1.5rem', lineHeight: 1.6 }}>
        ...
        <p style={{ fontSize: '11px', letterSpacing: '0.1em', color: 'rgba(255,255,255,0.35)', marginBottom: '10px' }}>SELECT PAPER</p>
        <div style={{ display: 'flex', gap: '10px', marginBottom: '2rem' }}>
          {(['P1', 'P2'] as Paper[]).map(p => (
            <button key={p} onClick={() => setPaper(p)} style={{
              ...
              background: paper === p ? 'rgba(55,138,221,0.12)' : 'rgba(255,255,255,0.03)',
            }}>
              <p style={{ fontSize: '16px', fontWeight: 600, color: '#fff', marginBottom: '4px' }}>{p === 'P1' ? 'Paper 1' : 'Paper 2'}</p>
        ...
        <div style={{ padding: '14px', background: 'rgba(55,138,221,0.08)', borderRadius: '12px', border: '1px solid rgba(55,138,221,0.2)', marginBottom: '2rem' }}>
          ...
          {paper === 'P1' ? (
            <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.45)', display: 'flex', flexDirection: 'column', gap: '4px' }}>
  ...
  if (phase === 'generating') return (
    <Screen>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
        <Spinner />
        <p style={{ fontSize: '16px', color: '#fff', marginTop: '1.5rem', marginBottom: '6px' }}>Generating extract…</p>
        <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.4)' }}>AI is writing a {paper === 'P1' ? 'fiction' : 'non-fiction'} passage and AQA-style questions</p>
      </div>
    </Screen>
  );
    ...
    <Screen>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        <div style={{ marginBottom: '1rem' }}>
          ...
          <h2 style={{ fontSize: '18px', fontWeight: 700, color: '#fff', marginBottom: '4px' }}>{session.title}</h2>
          <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.35)' }}>{session.questions.length} questions · {session.questions.reduce((s, q) => s + q.marks, 0)} marks total</p>
        </div>

        {/* Extract */}
        <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: '14px', padding: '16px', marginBottom: '1.5rem', border: '1px solid rgba(255,255,255,0.08)' }}>
          <button onClick={() => setShowExtract(e => !e)} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-ui)', marginBottom: showExtract ? '12px' : '0' }}>
            <span style={{ fontSize: '13px', fontWeight: 600, color: '#fff' }}>📄 Read the extract</span>
          ...
          {showExtract && (
            <div style={{ animation: 'fadeIn 0.2s ease' }}>
              ...
              <p style={{ fontSize: '14px', lineHeight: 1.85, color: 'rgba(255,255,255,0.82)', whiteSpace: 'pre-wrap' }}>
                {session.extract}
              </p>
            </div>
          )}
        ...
        <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.4)', marginBottom: '1rem', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '2rem' }}>
          {session.questions.map((q, i) => (
            <div key={q.number} style={{ background: 'rgba(0,0,0,0.3)', borderRadius: '14px', padding: '16px', border: '1px solid rgba(255,255,255,0.08)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                ...
                <span style={{ fontSize: '10px', padding: '2px 6px', borderRadius: '99px', background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.4)' }}>{q.marks} marks</span>
              </div>
              <p style={{ fontSize: '14px', color: 'rgba(255,255,255,0.85)', lineHeight: 1.5, marginBottom: '8px' }}>{q.question}</p>
              <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)', marginBottom: '10px', fontStyle: 'italic' }}>💡 {q.hint}</p>
              <textarea
                value={answers[q.number] ?? ''}
                onChange={e => setAnswers(prev => ({ ...prev, [q.number]: e.target.value }))}
                placeholder="Write your answer here…"
                rows={q.marks >= 20 ? 10 : q.marks >= 8 ? 6 : 3}
                style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '10px', color: '#fff', fontFamily: 'var(--font-ui)', fontSize: '14px', padding: '10px 12px', outline: 'none', resize: 'vertical', lineHeight: 1.6 }}
              />
              <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.25)', marginTop: '4px', textAlign: 'right' }}>
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '2rem' }}>
          {feedback.map((f, i) => {
            ...
            return (
              <div key={i} style={{ background: 'rgba(0,0,0,0.3)', borderRadius: '14px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.08)' }}>
                <div style={{ padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <span style={{ fontSize: '13px', fontWeight: 700, color: '#fff', fontFamily: 'var(--font-mono)' }}>Q{f.questionNum}</span>
                    {q && <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.35)' }}>{q.ao}</span>}
                  </div>
                ...
                <div style={{ padding: '14px 16px' }}>
                  ...
                  <div style={{ marginBottom: '12px' }}>
                    <p style={{ fontSize: '10px', letterSpacing: '0.08em', color: 'rgba(255,255,255,0.3)', marginBottom: '5px' }}>YOUR ANSWER</p>
                    <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.6)', lineHeight: 1.6, fontStyle: 'italic' }}>
                      "{answers[f.questionNum]?.slice(0, 200)}{(answers[f.questionNum]?.length ?? 0) > 200 ? '…' : ''}"
                    </p>
                  </div>
                  {/* Feedback */}
                  <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.75)', lineHeight: 1.7, whiteSpace: 'pre-wrap', background: 'rgba(255,255,255,0.03)', borderRadius: '8px', padding: '10px 12px', border: '1px solid rgba(255,255,255,0.06)' }}>
                    {f.feedback}
                  </div>
                </div>
              </div>
