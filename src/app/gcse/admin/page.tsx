  ...
  return (
    <Shell>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
        <button onClick={() => router.push('/gcse')} className="btn">Back</button>
        <div style={{ textAlign: 'center' }}>
          <p style={{ fontSize: '11px', letterSpacing: '0.12em', color: 'var(--fg-secondary)' }}>ADMIN</p>
          <h1 style={{ fontSize: '18px', fontWeight: 700, color: 'var(--fg)' }}>Lesson Manager</h1>
        </div>
        <button onClick={() => setShowCreate(s => !s)} style={{
          padding: '8px 14px', borderRadius: '8px', cursor: 'pointer',
          background: 'var(--green)', border: '2.5px solid var(--green-dark)', color: '#fff', boxShadow: '0 4px 0 var(--green-dark)',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '8px', marginBottom: '1.25rem' }}>
        {[
          ...
          { label: 'Published', value: counts.published, color: 'var(--green)' },
          { label: 'Drafts',    value: counts.drafts,    color: 'var(--orange)' },
        ].map(s => (
          <div key={s.label} style={{ background: 'var(--bg-secondary)', borderRadius: '10px', padding: '12px', border: '1px solid var(--bg-secondary)', textAlign: 'center' }}>
            <p style={{ fontSize: '22px', fontWeight: 700, color: s.color, fontFamily: 'var(--font-mono)' }}>{s.value}</p>
            <p style={{ fontSize: '10px', color: 'var(--fg-secondary)', letterSpacing: '0.08em' }}>{s.label.toUpperCase()}</p>
          </div>
        ))}
      </div>
      ...
      {showCreate && (
        <div style={{ background: 'rgba(0,232,122,0.06)', borderRadius: '14px', padding: '16px', border: '1px solid rgba(0,232,122,0.2)', marginBottom: '1.25rem', animation: 'fadeIn 0.2s ease' }}>
          <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--green)', marginBottom: '12px' }}>Generate new lesson</p>
          ...
          <div style={{ display: 'flex', gap: '6px', marginBottom: '10px' }}>
            {SUBJECTS.map(s => (
              <button key={s} onClick={() => { setCreateSubject(s); setCreateTopic(''); }} style={{
                ...
                border: `2px solid ${createSubject === s ? SUBJECT_COLOR[s] : 'var(--border-dark)'}`,
                background: createSubject === s ? SUBJECT_COLOR[s] + '15' : 'transparent',
                color: createSubject === s ? SUBJECT_COLOR[s] : 'var(--muted)',
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px', marginBottom: '10px' }}>
            {SUBJECT_TOPICS[createSubject]?.map(t => (
              <button key={t} onClick={() => setCreateTopic(t)} style={{
                ...
                border: `2px solid ${createTopic === t ? SUBJECT_COLOR[createSubject] : 'var(--border-dark)'}`,
                background: createTopic === t ? SUBJECT_COLOR[createSubject] + '15' : 'transparent',
                color: createTopic === t ? SUBJECT_COLOR[createSubject] : 'var(--muted)',
                transition: 'all 0.15s',
              }}>
                {t}
              </button>
          ...
          <input
            ...
            style={{ width: '100%', padding: '9px 12px', background: 'var(--bg-secondary)', border: '1px solid #fff', borderRadius: '8px', color: 'var(--fg)', fontFamily: 'var(--font-ui)', fontSize: '13px', outline: 'none', marginBottom: '10px' }}
          />
      ...
      <div style={{ display: 'flex', gap: '6px', marginBottom: '1rem', flexWrap: 'wrap' }}>
        {['all', 'maths', 'english', 'science'].map(f => (
          <button key={f} onClick={() => setFilterSubject(f)} style={{
            ...
            fontFamily: 'var(--font-ui)', fontSize: '11px',
            border: `2px solid ${filterSubject === f ? 'var(--blue)' : 'var(--border-dark)'}`,
        ...
        <div style={{ height: '28px', width: '1px', background: 'var(--border-dark)', margin: '0 2px' }} />
        {(['all', 'published', 'draft'] as const).map(f => (
          <button key={f} onClick={() => setFilterStatus(f)} style={{
            padding: '5px 12px', borderRadius: '99px', cursor: 'pointer',
            fontFamily: 'var(--font-ui)', fontSize: '11px',
            border: `2px solid ${filterStatus === f ? 'var(--green)' : 'var(--border-dark)'}`,
            background: filterStatus === f ? 'var(--green-light)' : '#fff',
            color: filterStatus === f ? 'var(--green-dark)' : 'var(--muted)',
            transition: 'all 0.15s',
          }}>
            {f.charAt(0).toUpperCase() + f.slice(1)}
          </button>
      ...
      {/* Lesson list */}
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '3rem 0', color: 'var(--fg-secondary)', fontSize: '14px' }}>
          No lessons yet. Click + New to generate one.
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {filtered.map(l => {
            const color = SUBJECT_COLOR[l.subject] ?? 'var(--blue)';
            return (
              <div key={l.id} style={{ background: 'rgba(0,0,0,0.3)', borderRadius: '12px', padding: '14px', border: `1px solid ${color}18`, borderLeft: `3px solid ${l.published ? color : '#fff'}` }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px', flexWrap: 'wrap' }}>
                  <span style={{ fontSize: '14px', fontWeight: 600, color: 'var(--fg)', flex: 1 }}>{l.topic}</span>
                  <span style={{ fontSize: '10px', padding: '2px 7px', borderRadius: '99px', background: color + '15', color: color + 'cc' }}>{l.subject}</span>
                  <span style={{ fontSize: '10px', padding: '2px 7px', borderRadius: '99px', background: l.published ? 'rgba(0,232,122,0.12)' : 'var(--bg-secondary)', color: l.published ? '#00e87a' : 'var(--muted)' }}>
                    {l.published ? 'Published' : 'Draft'}
                  </span>
                </div>
                <p style={{ fontSize: '11px', color: 'var(--fg-secondary)', marginBottom: '10px' }}>
