...
// ── Timed auto-hint ────────────────────────────────────
// Shows a gentle nudge if student hasn't interacted for N seconds
interface TimedHintProps {
  message:     string;
  afterSeconds: number;
  active:      boolean;   // only watch when card is active
  accentColor: string;
}

export function TimedHint({ message, afterSeconds, active, accentColor }: TimedHintProps) {
  const [show, setShow] = useState(false);
  const timerRef        = useRef<NodeJS.Timeout | null>(null);

  useEffect(() => {
    if (!active) { setShow(false); if (timerRef.current) clearTimeout(timerRef.current); return; }
    timerRef.current = setTimeout(() => setShow(true), afterSeconds * 1000);
