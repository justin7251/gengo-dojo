  ...
  if (phase === 'setup') return (
    <Screen color={color} onBack={() => router.push('/gcse/maths')}>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '1.5rem' }}>
        {(['Foundation', 'Higher'] as Tier[]).map(t => (
          <button key={t} onClick={() => { setTier(t); setSelectedSub(null); }} style={{
            ...
            background: tier === t ? `${color}15` : 'rgba(255,255,255,0.04)',
      ...
      <p style={{ fontSize: '11px', letterSpacing: '0.1em', color: 'rgba(255,255,255,0.3)', marginBottom: '8px' }}>
        SUBTOPIC <span style={{ color: 'rgba(255,255,255,0.2)', fontWeight: 400 }}>— optional, leave blank for mixed</span>
      </p>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '1.5rem' }}>
        <button onClick={() => setSelectedSub(null)} style={{
          ...
          background: selectedSub === null ? `${color}15` : 'rgba(255,255,255,0.04)',
        ...
        {filteredSubs.map(s => (
          <button key={s.id} onClick={() => setSelectedSub(s.id)} style={{
            ...
            background: selectedSub === s.id ? `${color}15` : 'rgba(255,255,255,0.04)',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '2rem' }}>
          {questions.map(q => (
            <div key={q.number} style={{
              ...
              {/* Answer box */}
              <label style={{ fontSize: '11px', letterSpacing: '0.06em', color: 'rgba(255,255,255,0.3)', display: 'block', marginBottom: '6px' }}>
                SHOW YOUR WORKING
              </label>
              <textarea
              ...
              <div key={i} style={{ background: 'rgba(0,0,0,0.3)', borderRadius: '14px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.07)' }}>
                ...
                <div style={{ padding: '12px 14px' }}>
                  ...
                  <div style={{ marginBottom: '10px' }}>
                    <p style={{ fontSize: '10px', letterSpacing: '0.08em', color: 'rgba(255,255,255,0.3)', marginBottom: '6px' }}>MODEL WORKING</p>
                    <pre style={{ fontSize: '13px', color: 'rgba(255,255,255,0.75)', lineHeight: 1.7, whiteSpace: 'pre-wrap', fontFamily: 'var(--font-mono)', background: 'rgba(255,255,255,0.04)', padding: '10px 12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.07)', margin: 0 }}>
                      {r.working}
                    </pre>
                  </div>
                  {/* Feedback */}
                  <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.65)', lineHeight: 1.6 }}>{r.feedback}</p>
                </div>
              </div>
            );
          })}
...
function Screen({ children, color, onBack }: { children: React.ReactNode; color: string; onBack: () => void }) {
  return (
    <main style={{
      minHeight: '100vh',
      background: '#050a18',
      backgroundImage: `radial-gradient(ellipse at top, ${color}12 0%, #050a18 60%)`,
      display: 'flex', flexDirection: 'column',
      padding: '1.5rem 1.25rem 2.5rem', fontFamily: 'var(--font-ui)',
      position: 'relative', overflow: 'hidden',
    }}>
      <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', backgroundImage: `linear-gradient(${color}18 1px,transparent 1px),linear-gradient(90deg,${color}18 1px,transparent 1px)`, backgroundSize: '40px 40px', opacity: 0.4 }} />
